...
- Ban fake interaction flow for tempban permission, duplicate-active checks, Discord ban failure, DB record, scheduling, notification ordering, the one-hour spam-message deletion default, and legacy `delete_message_days` compatibility.
...
- InviteGuard reward notification DM: after reward points are actually credited, the inviter receives a Components v2 DM (Container with locale-backed title/body, separators, `resources/images/invite_reward.png` MediaGallery attachment, total-invite-count footer, plus a top-level ActionRow link button jumping to the leaderboard panel message). Sending is gated on `reward_notification_enabled` and a valid leaderboard panel (runtime or configured channel + message id); notification failures (Forbidden, HTTPException, missing image) never affect attribution or points. Covered: DM view structure and link URL, invalid-panel and disabled-config skips, pooled per-inviter `body_pooled` summary notifications, Forbidden isolation, zero-points skip, and attribution-then-points-then-DM ordering.
- InviteGuard monthly leaderboard: a shared panel shows the current top ten, lifetime rankings, and the previous calendar month's settled champion. The initial lifetime totals are imported into the activation month once; later credits use attribution timestamps and are stored atomically with lifetime counts. `Europe/Berlin` midnight boundaries handle DST, frozen monthly results survive restarts, Shop transaction keys prevent duplicate payouts, and notification claims prevent automatic duplicate DMs after an uncertain send. Tests cover opening-balance import, year/DST boundaries, bot filtering, Top 10 rewards, offline catch-up, account lookup failures, payment recovery, and DM failure isolation.
