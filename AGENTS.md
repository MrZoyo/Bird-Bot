...
- 自动化单测：`python -m pytest`（需要 `uv sync --extra test`；覆盖配置模板、runtime cog 元数据、日志 helper、临时 JSON→YAML 配置迁移 smoke，以及可离线验证的 DB manager smoke）。
...
## 测试指引
- 已有 `pytest` 自动化 smoke tests 覆盖配置模板、runtime cog 元数据、日志 helper、临时 JSON→YAML 配置迁移和纯 DB manager；涉及真实 Discord 交互时仍需在测试服逐条验证命令与按钮，流程以 `REFACTORING_TEST_CHECKLIST.md` 为准。
