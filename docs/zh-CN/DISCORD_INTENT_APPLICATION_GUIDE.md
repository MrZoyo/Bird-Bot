...
说明：当前运行配置默认日志轮转保留 14 个日备份文件，活跃组队邀请保留至结束，已结束且消息同步完成的历史记录保留 14 天；未确认发布及更新失败记录按恢复状态处理；但由于运营方可能保留手动备份或按实际服务器政策调整日志保留，因此这里按保守口径填写 `No`。如提交人确认生产环境严格保证所有消息内容日志和备份均不超过 30 天，也可以改为 `Yes`，但需要与实际运维策略保持一致。
...
4. 对于组队展示，Bot 保存组队文本、频道 ID、用户 ID、语音频道 ID、邀请状态和消息关联；活跃邀请持续保留，已结束且同步完成的历史记录在 14 天后清理。其他持久化文本仅限功能必要内容，例如用户主动设置的签名、工单关闭原因、抽奖描述等。
```
