...
同一面板上方显示当月前10名，下方显示总榜，两者由分割线区分；“更新于”右侧显示上月已结算冠军。两个榜单都排除机器人，并按邀请总数降序、用户 ID 升序处理同分。
...
月榜私信沿用奖励通知开关、图片及榜单链接。私信失败不影响积分；发送结果保存为 `sent` / `failed`。发送过程中进程中断会保留 `sending` 状态供运营核查，避免自动重试造成重复私信。
