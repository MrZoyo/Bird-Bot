...
助力用户默认额外计入 30 小时语音时长，用于购买和续费折扣。`privateroom.booster_discount_hours` 可覆盖该值，设为 0 可关闭加成；助力身份由 `role.signature.helper_role_id` 指定的角色判断。
...
同一面板上方显示总榜，下方显示当月前10名，两者由分割线区分；“更新于”右侧显示上月已结算冠军。两个榜单都排除机器人，并按邀请总数降序、用户 ID 升序处理同分。

月榜使用 `monthly_timezone` 指定的 IANA 时区，默认为 `Europe/Berlin`。首次启用时，现有累计邀请数一次性计入启用月份；之后单人和池化归因在增加总数的同一事务中记录新增积分对应的邀请数，以归因入账时间划分自然月。首次导入及其月份会持久化，重启不会重复导入。修改已启用的时区需要单独迁移。

每月1日当地00:00结算上月：冠军200积分、亚军150积分、季军100积分，第4–10名各60积分；这是逐次邀请奖励之外的月榜奖励。月初冻结获奖名单，积分流水使用唯一奖励键，重启或重试不会重复到账；离线错过的月份会在启动后补结算。空月份无冠军、无奖励。无法确认候选人账号时延后结算，避免误发名次奖励。

月榜私信沿用奖励通知开关及榜单链接，并明确说明月榜积分是额外奖励。冠军、亚军、季军和第4–10名分别使用 `resources/images/` 下的 `invitation_no1.png`、`invitation_no2.png`、`invitation_no3.png` 和 `invitation_no4-10.png`。私信失败不影响积分；发送结果保存为 `sent` / `failed`。发送过程中进程中断会保留 `sending` 状态供运营核查，避免自动重试造成重复私信。
