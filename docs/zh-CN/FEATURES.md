...
CreateInvitationCog 创建带有作者当前语音房直达链接的组队消息。它支持显式 `/invt` 调用，也支持在普通消息中自动检测配置的组队短语。

邀请可以包含作者签名。作者不在语音频道时，回复会链接到配置的房间入口频道。忽略列表可以排除指定用户和文字频道。语音房控制面板和邀请面板共用满员更新路径，因此最终状态一致。
