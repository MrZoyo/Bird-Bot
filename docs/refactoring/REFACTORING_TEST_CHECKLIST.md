...
- [x] 发送组队邀请后，从房间面板点击 Full；组队消息样式变满员，展示板移除或更新正确。
  - 2026-09-13：邀请生命周期修复在独立测试库验证。用户实际点击一条已存在约 16 分钟且经历机器人重启的邀请按钮，交互已应答，消息按钮移除、看板清空。随后用户通过普通消息依次发布 A/B，A 自动满员，再从重启恢复的房间面板结束 B；真实交互日志、数据库状态及 Discord 消息读回一致。非发布者房间成员、旧点击隔离另有离线及真实 REST 消息测试，未冒充第二账号的真人交互。全量离线回归 199 passed；加密测试库副本迁移前后完整性通过，47 张无关业务表内容一致。本轮只验证测试服，未部署生产。
