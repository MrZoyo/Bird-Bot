...
## 2026-08-16 组队提示与成就面板 / 2.0.4

- 组队自动检测保留“标记 + 人数”作为必需入口；只有命中后才回看标记前是否为独立的 `1`、`１` 或 `一`。`1q4`、`一等全世界` 使用单人柔和提示，`稍微一等`、`1等`、`一q` 不触发。
- 用户不在语音频道时，普通回复改为“本频道不允许私拉，先创个房间吧~”；单人状态改为“不如你先来发车~”。
- 6 字符消息静默忽略规则保持不变，并将不区分大小写的 `hks` 加入 `flex`、`rank`、`aram` 例外。
- 当前 `/achievements` 列表迁移到 Components v2，以原生分割线区分成就分类，右上角显示大头像；头像按“用户自定义头像 → Bot 自定义头像 → 用户默认头像”降级。测试服样板已确认采用大头像方案。
- 新增组队关键词 flow 和成就面板结构测试；全量自动化基线为 `159 passed, 1 warning`。
- 版本号推进到 `2.0.4`。

---

## 2026-08-15 `@everyone` 自动防御 / 2.0.3

- 新增可选的真实 `@everyone` 自动防御：普通成员触发后沿用现有临时封禁链路，默认封禁 `1d`，并删除最近 `1h` 的历史消息。
- 豁免服主、Discord 管理员、Ban 管理员用户/身份组，以及能在 `main.admin_channel_id` 发言的成员；只含文字但未真正触发群体提及的消息不会误判。
- 临时封禁私信新增账户安全提示：误点垃圾广告时，应先启用 Discord 多重认证再重新加入服务器。
- 数据库写入失败时立即回滚 Discord 封禁；重复封禁会复用并重新激活现有 user/guild 记录，避免唯一约束导致自动防御留下永久封禁。
- 配置模板使用 `delete_message_seconds: 3600`，运行时继续兼容旧 `delete_message_days`。全量测试基线为 `155 passed, 1 warning`，并已在测试服验证真实封禁、通知、私信、定时解封及手动解封后的状态清理。
- 版本号推进到 `2.0.3`。
...
- InviteGuard 手动入口收敛为 `/invite_sync`、`/invite_check_user <member>`、`/invite_create_embed <channel>`；项目内“owner / 管理员”默认按能访问 `main.admin_channel_id` 的用户理解。邀请创建者白名单还会自动包含 bot owner、Discord administrator 和能访问管理频道的成员。
- 离线覆盖新增 / 扩展 `tests/test_invite_guard.py` 和 `tests/test_invite_guard_db.py`：过期删除、dry-run 不删除、邀请 code 白名单、创建者白名单、`created_at` 为空跳过、单条删除失败不中断、邀请链接同步、成员加入归因、重复加入不重复计数、忽略 / 模糊来源不计数、Components v2 排行榜刷新 / 重建命令、有效邀请写入 Shop 积分余额和交易记录。
- 邀请排行榜面板使用 Components v2 紫色容器，显示 Top 15、`UTC+2 HH:MM` 更新时间、分割线、emoji 排名和 bot 头像缩略图；`reward_points_per_invite` 默认 60，设为 0 可关闭奖励。
...
- 当前自动化基线：`./.venv/Scripts/python.exe -m pytest -q` 为 `120 passed, 1 warning`；`ruff check bot tests tools`、`compileall bot tests tools`、`tools/check_locales.py` 均通过。
...
- 当前自动化基线：`./.venv/Scripts/python.exe -m pytest -q` 为 `120 passed, 1 warning`。
...
- 验证：上述新增测试 targeted pytest 均通过；全量 `pytest` 当时为 `80 passed, 1 warning`；`ruff`、`compileall`、locale check、`pip check`、`uv lock --check`、`uv sync --frozen --dry-run --extra test --extra lint --python 3.12.3`、`git diff --check` 均通过。`uv sync` dry-run 继续提示 WSL 下会替换 Windows `.venv`，未实际同步。
...
- `bot/utils/log_helpers.py` 新增 `fmt_user` / `fmt_channel` / `fmt_role`；用户日志输出 `昵称 / 用户名 (id)`（昵称和用户名相同则省略重复用户名），频道 / 身份组输出 `name (id)`；raw id 兜底为 `unknown (id)`，不为日志补做 Discord API fetch。
...
- 2026-04-29 补充：关键词检测日志改为 `fmt_user(message.author)` + `fmt_channel(message.channel)`，输出用户昵称 / 用户名 / id 和文本频道的 `name (id)`；`tests/test_invitation_full_message.py` 覆盖该日志格式。
- 2026-04-29 补充 2：`fmt_user()` 改为用户昵称 / 用户名 / id 三段式，典型格式 `昵称 / 用户名 (id)`；频道 / 身份组仍为 `name (id)`；所有数字 id 使用英文括号。
...
- 当时自动化 baseline 提升到 `80 passed, 1 warning`；新增测试均为离线 fake/fixture，不使用真实 Discord、不触碰真实 DB。
...
**跨表访问处理**：`monthly_achievements` 表归属 achievement 领域，但 privateroom_cog 只依赖 `PrivateRoomDatabaseManager`；为保持单 manager 依赖、避免无谓引入 `AchievementDatabaseManager`，该方法先放在 privateroom 侧。后续跨 manager 引用成常态时再统一梳理。
