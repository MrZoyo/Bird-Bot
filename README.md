...
5. In `main.yaml`, use `features` to enable only the modules you need.
   ...
   - For encrypted database storage, set `DCGSH_DB_KEY_FILE` to a secret file outside the repository. For first-run key-file generation, also set `DCGSH_DB_CREATE_KEY_FILE=1`, run the migration with `python tools/encrypt_database.py`, then remove the create flag and set `DCGSH_DB_REQUIRE_ENCRYPTION=1` in production.
