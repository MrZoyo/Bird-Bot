...
discord.py, aiosqlite, aiohttp, requests, aiofiles, pillow, matplotlib, tqdm, PySimpleGUI

---
## Setup
0. Clone the repository and enter the directory.
1. Recommended: create the virtual environment with the locked Python version in `.python-version`.
   - `uv venv --python 3.12.3`
   - `source .venv/bin/activate`
2. Install the locked dependency set.
   - `uv pip sync requirements.lock`
3. `requirements.txt` remains the human-maintained direct dependency list. `requirements.lock` is the reproducible lock file generated by `uv pip compile requirements.txt -o requirements.lock`.
4. Copy only the required `config_*.json.example` files in `bot/config/`, rename them to `config_*.json`, and fill in the values.
5. In `config_main.json`, use `features` to enable only the modules you need.
...
7. In the Discord Developer Portal, enable the privileged intents required by this bot. The current runtime requests `Intents.all()`, so the safe setup is to enable `Server Members Intent`, `Presence Intent`, and `Message Content Intent`.
8. Run `python run.py`. If you are using a Linux server, you can use `nohup python3 run.py &` to run the bot in the background.
9. Invite the bot to your server and give it the necessary permissions.(Required permissions: bot, application command, administrator)
10. For updating the bot, you can use the `git pull` command to update the bot to the latest version, then rerun `uv pip sync requirements.lock`.
11. For `tickets_new_cog`, use `/tickets_init` to initialize the ticket system. Please check function introduction for more details.
...
## Update Log Latest
### V1.9.1 - 2026-04-14
- Locked the Python runtime and dependency environment with `.python-version` and `requirements.lock`.
