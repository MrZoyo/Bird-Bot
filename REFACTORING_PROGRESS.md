...
**P1-3b 全量包化已完成**：`bot/cogs/` 顶层现在只剩 `__init__.py` 和包目录，不再有平面 `*_cog.py`。最近三条相关 commit：
...
**下一棒默认**：先做 `service.py` 横扫评估（只调研、列候选和收益，不直接抽代码）。如果评估收益不够，转 **P2-1 数据库连接复用**，但 P2-1 开工前必须先设计 `close()` 生命周期和 bot 关闭钩子。

**环境验证规则**：环境 / import / 启动验证必须提权到沙箱外跑真实环境。当前外部 Windows `.venv` 可启动，且已有 `discord 2.7.1` / `aiosqlite 0.22.1`，但缺 `ruamel.yaml`；`uv pip sync requirements.lock --python ./.venv/Scripts/python.exe` 在 WSL 下未识别该 Windows venv。下次做运行期验证前，先解决 `.venv` 依赖同步或换一个完整环境。
...
**2026-04-24 后续规划**（用户决定）：P1-3 扩展为 **§P1-3b 全量 cog 包化 + games 聚合** + **§P1-3c tickets_new → tickets rename**（见 PLAN 对应章节）。该规划已于 2026-04-25 全部收官；service.py 抽离后置，当前默认下一棒是横扫评估。
...
5. **service.py 横扫评估（当前默认下一棒）**：所有 cog 都在标准包骨架下后，先列候选和收益，不直接抽代码。
...
P1-3 / P1-3b 拆包完 ✅，P1-3c rename 也完成（2026-04-24）。现在可以回来评估 DB 层，但建议先做一次 `service.py` 横扫评估，避免马上进入更重的生命周期改造。
...
2. `git log --oneline -10`（确认最近三条：`c774018` / `a1ceefa` / `777d3e2`）
...
5. 决定路径：
   ...
   - **下一棒默认**：service.py 横扫评估（只调研）；若跳过，进入 P2-1 数据库连接复用生命周期设计。
