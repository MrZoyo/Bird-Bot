...
- 当前自动化基线：`./.venv/Scripts/python.exe -m pytest -q` 为 `80 passed, 1 warning`。
...
- 验证：上述新增测试 targeted pytest 均通过；全量 `pytest` 当前 `80 passed, 1 warning`；`ruff`、`compileall`、locale check、`pip check`、`uv lock --check`、`uv sync --frozen --dry-run --extra test --extra lint --python 3.12.3`、`git diff --check` 均通过。`uv sync` dry-run 继续提示 WSL 下会替换 Windows `.venv`，未实际同步。
...
- 当前自动化 baseline 提升到 `80 passed, 1 warning`；新增测试均为离线 fake/fixture，不使用真实 Discord、不触碰真实 DB。
