...
4. **P1-3b 第三档**（**当前下一棒**）：shop（**注意 persistent view** — `shop_cog.py:647` `self.bot.add_view(self.checkin_view)`，`custom_id` 需在包化 commit 前 grep 确认是字符串字面量）+ role（1151 行，4 View + 1 Modal → 完整包）。
